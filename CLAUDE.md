...
| Line limit | `mise run check:lines` → [`scripts/check-line-limit.mjs`](./scripts/check-line-limit.mjs), the single authority: ≤500 raw lines over `.ts .tsx .mts .mjs .js` **and** `.css`, minus its skip list (build output, `vendor/`, `.tmp/`, `bench/`, `site/`, `design/`, `.claude/`, `types/generated*`, `scripts/main.ts`) |
...
| Capability probe | `mise run probe` → 104 `Script.Eval` expressions (`server/probe/probe-catalog.ts`) → `types/generated-probe.json` → `types/generated.d.ts`. Excluded from the device compile; absences surface as `probe-absent-api`. Severity follows provenance: **error** from the active device, **warn** otherwise |
