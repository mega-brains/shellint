...
that `extends` `tsconfig.shelly.json`, so a file that drifts out of the
dialect fails the bench run loudly.

They are excluded from `tsconfig.shelly.json`'s `include` (which names
`scripts/main.ts` only) and from `check:lines` — a benchmark input needs bulk
