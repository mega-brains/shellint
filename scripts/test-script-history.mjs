...
try {
  ...
  snapshotBeforeWrite("var a = 1;", "var a = 2;", t);
  ...
  snapshotBeforeWrite("var a = 2;", "var a = 2;", (t += STEP_MS));
  ...
  snapshotBeforeWrite("var a = 1;", "var a = 3;", (t += STEP_MS));
  ...
  snapshotBeforeWrite("var a = 3;", "var a = 4;", (t += STEP_MS));
  ...
  rmSync(HISTORY_FILE, { force: true });
  t = Date.now();
  snapshotBeforeWrite("var b = 1;", "var b = 2;", t);
  snapshotBeforeWrite("var b = 2;", "var b = 3;", t + 1000);
  rows = listScriptHistory();
  if (rows.length !== 1) {
    fail(`expected coalescing to suppress a snapshot within the window, got ${rows.length} rows`);
  }
  const coalesced = readScriptHistoryRow(rows[0].id);
  ...
  snapshotBeforeWrite("var b = 3;", "var b = 4;", t + STEP_MS);
  rows = listScriptHistory();
  ...
  checkpointNow("var b = 4;", t + STEP_MS + 500);
  rows = listScriptHistory();
  ...
  const dupe = checkpointNow("var b = 4;", t + STEP_MS + 900);
  if (dupe !== null) fail("checkpointNow should dedupe against an identical most-recent row");
  if (listScriptHistory().length !== 3) fail("deduped checkpoint should not add a row");

  // 10-row FIFO cap, each snapshot spaced outside the coalescing window.
  rmSync(HISTORY_FILE, { force: true });
  t = Date.now();
  for (let i = 0; i < 12; i++) {
    snapshotBeforeWrite(`var a = ${i};`, `var a = ${i + 1};`, t + i * STEP_MS);
  }
  rows = listScriptHistory();
